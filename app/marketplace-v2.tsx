...
import {
  ...
  Sparkles, Star, Tag, Trash2, TrendingDown, UploadCloud,
...
function AuthModal({close,complete,lang}:{close:()=>void;complete:()=>void;lang:Lang}){
  const [mode,setMode]=useState<"login"|"signup">("login"); const [email,setEmail]=useState(""); const [password,setPassword]=useState(""); const [name,setName]=useState(""); const [error,setError]=useState("");
  const submit=(e:FormEvent)=>{e.preventDefault();const identity=email.trim().toLowerCase();if(mode==="login"&&identity==="arnis@arnis.com"&&password==="arnis"){try{localStorage.setItem("velora-account",JSON.stringify({name:"Arnis Mulliqi",email:"arnis@arnis.com"}));}catch{}complete();return}if(mode==="login"){setError("Emaili ose fjalëkalimi nuk është i saktë.");return}if(!identity.includes("@")||password.length<6||!name.trim()){setError("Plotëso të dhënat me email të vlefshëm dhe fjalëkalim me së paku 6 karaktere.");return}try{localStorage.setItem("velora-account",JSON.stringify({name:name.trim(),email:identity,password}));}catch{}complete()};
  return <motion.div className="v2-modal-bg v2-auth-modal-bg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={close}><motion.section className="v2-auth-modal" initial={{opacity:0,y:18,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:18,scale:.98}} onClick={e=>e.stopPropagation()} role="dialog" aria-modal="true" aria-label={mode==="login"?"Kyçu":"Krijo llogari"}><button className="v2-icon v2-auth-close" onClick={close} aria-label="Mbyll"><X/></button><Brand/><span>VELORA ACCOUNT</span><h1>{mode==="login"?"Mirë se vjen.":"Krijo llogarinë."}</h1><p>{mode==="login"?"Kyçu për të ruajtur produktet, për të dërguar oferta dhe për të shitur.":"Ruaj produktet, publiko shpallje dhe menaxho shitjet e tua."}</p><form onSubmit={submit}>{mode==="signup"&&<label>Emri i plotë<input autoComplete="name" value={name} onChange={e=>setName(e.target.value)} placeholder="Emri dhe mbiemri"/></label>}<label>Email<input autoComplete={mode==="login"?"username":"email"} type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder={mode==="login"?"arnis@arnis.com":"emri@email.com"}/></label><label>Fjalëkalimi<input autoComplete={mode==="login"?"current-password":"new-password"} type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder={mode==="login"?"p.sh. arnis":"Minimum 6 karaktere"}/></label>{error&&<small className="v2-auth-error">{error}</small>}<button className="v2-pill dark wide">{mode==="login"?"Kyçu":"Krijo llogari"}<ArrowRight/></button></form><div className="v2-auth-switch"><span>{mode==="login"?"Nuk ke llogari?":"Ke tashmë llogari?"}</span><button onClick={()=>{setMode(mode==="login"?"signup":"login");setError("")}}>{mode==="login"?"Regjistrohu":"Kyçu"}</button></div><small className="v2-auth-note">Llogaria testuese: <b>arnis@arnis.com</b> / <b>arnis</b>. Për prodhim lidhet backend i verifikuar.</small></motion.section></motion.div>
}
...
function AppHeader({cartCount,noteCount,notes,setNotes,lang,setLang,signedIn,onSignIn,onSignOut}:{cartCount:number;noteCount:number;notes:Note[];setNotes:(v:Note[])=>void;lang:Lang;setLang:(l:Lang)=>void;signedIn:boolean;onSignIn:()=>void;onSignOut:()=>void}){
  ...
  const [term,setTerm]=useState(params.get("q")||""); const [menu,setMenu]=useState(false); const [profile,setProfile]=useState(false); const [auth,setAuth]=useState(false); const [searchOpen,setSearchOpen]=useState(false); const [notifications,setNotifications]=useState(false); const [afterLogin,setAfterLogin]=useState<string|null>(null);
  useEffect(()=>{
  ...
  useEffect(()=>{if(params.get("login")==="sell"&&!signedIn){setAfterLogin("/sell");setAuth(true)}},[params,signedIn]);
  return <>
    <header className="v2-header">
      <div className="v2-header-inner">
        ...
        <nav className="v2-nav"><Link href="/explore">Discover</Link><Link href="/brands">Brands</Link><Link href="/stories">Stories</Link><Link href="/sell">Sell</Link></nav>
      ...
      <div className="v2-catbar"><Link href="/explore?sort=new">New today</Link><Link href="/brands">Designers</Link>{categories.slice(1).map(c=><Link key={c} href={`/explore?category=${encodeURIComponent(c)}`}>{c}</Link>)}<Link href="/authentication"><ShieldCheck/>Authenticated</Link><Link href="/explore?sort=drop"><Zap/>Price drops</Link></div>
    </header>
    ...
    <AnimatePresence>{auth&&<AuthModal lang={lang} close={()=>{setAuth(false);setAfterLogin(null)}} complete={()=>{onSignIn();setAuth(false);if(afterLogin)router.push(afterLogin)}}/>}</AnimatePresence>
  </>
}
...
function HomePage({saved,toggle,add,signedIn}:{saved:number[];toggle:(id:number)=>void;add:(id:number)=>void;signedIn:boolean}){
  ...
  return <main>
    ...
    <section className="v2-market-gateway"><div className="v2-gateway-head"><span>GJITHÇKA NË NJË VEND</span><h2>Ku dëshiron të fillosh?</h2></div><nav>{[[Search,"Eksploro tregun","Produkte të kuruara","/explore"],[Sparkles,"Të reja sot","Sapo publikuar","/explore?sort=new"],[BadgeCheck,"Brendet","A–Z dhe koleksione","/brands"],[ShieldCheck,"Autentikimi","Si ndërtohet besimi","/authentication"],[LayoutDashboard,"Store Mode","Për shitës profesionalë","/professional-sellers"],[FileCheck2,"VELORA Stories","Guida dhe editorial","/stories"]].map(([Icon,title,text,href])=><Link href={String(href)} key={String(title)}>{typeof Icon!=="string"&&<Icon/>}<span><b>{String(title)}</b><small>{String(text)}</small></span><ChevronRight/></Link>)}</nav></section>
    <section className="v2-live-pulse"><div><span><i/> PO NDODH TANI</span><b>18 produkte të reja sot</b><small>·</small><b>6 ulje çmimesh</b><small>·</small><b>3 artikuj në ofertë</b></div><Link href="/explore?sort=new">Shiko tani<ArrowRight/></Link></section>
    ...
    <section className="v2-drop-banner v2-section"><div><span>DROPS TË JAVËS</span><h2>Pjesë që nuk rrinë gjatë.</h2><p>Koleksione të vogla, produkte të rralla dhe çmime që lëvizin me tregun.</p><Link className="v2-pill light" href="/explore?sort=new">Eksploro drop-et<ArrowRight/></Link></div><aside><b>24</b><small>persona po shikojnë produktet e reja</small></aside></section>
    <Rail title="Të reja sot" items={[products[9],products[13],products[18],products[23],products[26],products[17]]} saved={saved} toggle={toggle} add={add}/>
    ...
    <section className="v2-sell-banner v2-section"><div><span>YOUR WARDROBE, REVALUED</span><h2>Give exceptional pieces their next chapter.</h2><p>Photograph it, review the smart listing, publish. You remain in control.</p><Link href="/sell" className="v2-pill dark">Start selling<ArrowRight/></Link></div><div className="v2-sell-preview"><ProductImage p={products[0]}/><span><WandSparkles/>Smart listing assistant</span><b>Bottega Veneta detected</b><small>Suggested range €1,180–€1,340</small></div></section>
  </main>
}
...
function SellPage({publish,signedIn}:{publish:(p:ListingDraft)=>void;signedIn:boolean}){
  const router=useRouter(); const input=useRef<HTMLInputElement>(null); const [step,setStep]=useState(1); const [preview,setPreview]=useState(""); const [cover,setCover]=useState(""); const [title,setTitle]=useState(""); const [brand,setBrand]=useState("Bottega Veneta"); const [category,setCategory]=useState("Bags"); const [condition,setCondition]=useState("Si i ri"); const [gender,setGender]=useState("Femra"); const [size,setSize]=useState(""); const [retailPrice,setRetailPrice]=useState(1600); const [price,setPrice]=useState(0); const [published,setPublished]=useState(false); const [activeShot,setActiveShot]=useState("Përpara"); const [captured,setCaptured]=useState<string[]>([]);
  ...
  useEffect(()=>{if(step===3&&!price){const r=ratios[condition]||ratios["Mirë"];setPrice(Math.round(retailPrice*(r[0]+r[1])/2))}},[step,price,condition,retailPrice]);
  const file=(f?:File)=>{if(!f)return;const img=new Image();const src=URL.createObjectURL(f);img.onload=()=>{const max=1100,scale=Math.min(1,max/Math.max(img.width,img.height));const canvas=document.createElement("canvas");canvas.width=Math.round(img.width*scale);canvas.height=Math.round(img.height*scale);canvas.getContext("2d")?.drawImage(img,0,0,canvas.width,canvas.height);const data=canvas.toDataURL("image/jpeg",.78);URL.revokeObjectURL(src);setPreview(data);if(!cover||activeShot==="Përpara")setCover(data);setCaptured(v=>v.includes(activeShot)?v:[...v,activeShot])};img.src=src};
  if(published)return <main className="v2-sell-page"><div className="v2-publish-success"><Check/><span>LISTING PUBLISHED</span><h1>Your piece is live.</h1><p>It is now visible in your wardrobe and ready for buyers.</p><div><button className="v2-pill dark" onClick={()=>router.push("/profile")}>View listing</button><button className="v2-pill outline" onClick={()=>{setPublished(false);setStep(1);setPreview("");setCaptured([])}}>Sell another</button></div></div></main>;
  return <main className="v2-sell-page"><div className="v2-sell-top"><Link href="/" aria-label="Close selling flow"><X/></Link><Brand/><span>Draft saved locally</span></div><div className="v2-steps">{[1,2,3,4].map(n=><span key={n} className={step>=n?"active":""}>{step>n?<Check/>:n}</span>)}</div><motion.section key={step} initial={{opacity:0,x:14}} animate={{opacity:1,x:0}} className="v2-sell-step">
    {step===1&&<><span>STEP 1 OF 4 · GUIDED CAPTURE</span><h1>Fotografoje si një ekspert.</h1><p>Zgjidh çdo kënd dhe ndiq kornizën. Fotot e paqarta, screenshot-et dhe imazhet e ripërdorura dërgohen për kontroll.</p><input ref={input} hidden type="file" accept="image/*" capture="environment" onChange={e=>{file(e.target.files?.[0]);e.currentTarget.value=""}}/><div className="v2-guided-capture"><div className="v2-camera-frame" style={preview?{backgroundImage:`url(${preview})`}:undefined}><span/><span/><span/><span/>{preview?<><i>{activeShot}</i><b><Check/>Cilësia e pranueshme</b></>:<button onClick={()=>input.current?.click()}><Camera/><b>Hap kamerën</b><small>{activeShot} · dritë natyrale · pa filtra</small></button>}</div><aside><div><b>Fotot e detyrueshme</b><small>{captured.length} nga {shots.length} të përfunduara</small></div>{shots.map(x=><button key={x} className={`${activeShot===x?"active":""} ${captured.includes(x)?"done":""}`} onClick={()=>{setActiveShot(x);if(!captured.includes(x))setPreview("")}}><span>{captured.includes(x)?<Check/>:<Camera/>}{x}</span><ChevronRight/></button>)}<button className="v2-capture-action" onClick={()=>input.current?.click()}><UploadCloud/>{captured.includes(activeShot)?"Ribëje fotografinë":"Bëje këtë fotografi"}</button></aside></div><div className="v2-photo-safety"><ShieldCheck/><span><b>Kontroll automatik i cilësisë dhe origjinës</b><small>Kontrollohen turbullira, ndriçimi, prerja, rezolucioni, metadata dhe ngjashmëria me shpallje të tjera.</small></span></div></>}
    ...
    <div className="v2-sell-nav"><button disabled={step===1} onClick={()=>setStep(step-1)}>Prapa</button><button disabled={(step===1&&captured.length<shots.length)||(step===2&&(!title.trim()||!gender||!size))||(step===3&&(!retailPrice||!price))} className="v2-pill dark" onClick={()=>{if(step<4)setStep(step+1);else{publish({id:Date.now(),title:title||"Produkt i ri",brand,category,condition,gender,size,retailPrice,price:price||suggested[0],image:cover||preview,publishedAt:new Date().toISOString()});setPublished(true)}}}>{step===4?"Publiko shpalljen":"Vazhdo"}<ArrowRight/></button></div>
  </motion.section></main>
}
...
function Footer(){return <footer className="v2-footer"><div><section><Brand/><p>The considered marketplace for authenticated fashion, modern icons and remarkable second lives.</p><span className="v2-footer-locale">Kosovë · EUR · Shqip</span></section>{[["Bli","Të reja","Brendet","Autentikimi","Mbrojtja e blerësit"],["Shit","Si të shesësh","Store Mode","Paneli i shitësit","Produktet e ndaluara"],["Ndihmë","FAQ","Kontakt","Kthimet","Dërgesa"],["VELORA","Stories","Rreth nesh","Privatësia","Kushtet"]].map(col=><section key={col[0]}><b>{col[0]}</b>{col.slice(1).map(x=><Link key={x} href={x==="Të reja"?"/explore?sort=new":x==="Brendet"?"/brands":x==="Autentikimi"?"/authentication":x==="Mbrojtja e blerësit"?"/buyer-protection":x==="Si të shesësh"?"/sell":x==="Store Mode"?"/professional-sellers":x==="Paneli i shitësit"?"/dashboard":x==="Produktet e ndaluara"?"/prohibited-items":x==="FAQ"?"/faq":x==="Kontakt"?"/contact":x==="Kthimet"?"/returns":x==="Dërgesa"?"/shipping-policy":x==="Stories"?"/stories":x==="Privatësia"?"/privacy":x==="Kushtet"?"/terms":"/"}>{x}</Link>)}</section>)}</div><div><span>© 2026 VELORA Marketplace · Demo</span><nav><Link href="/privacy">Privatësia</Link><Link href="/terms">Kushtet</Link><Link href="/buyer-protection">Mbrojtja</Link><Link href="/contact">Kontakt</Link></nav></div></footer>}

function MobileNav({cart,signedIn}:{cart:number;signedIn:boolean}){const p=usePathname();return <nav className="v2-mobile-nav">{[[Home,"Home","/"],[Search,"Discover","/explore"],[Plus,"Sell",signedIn?"/sell":"/?login=sell"],[MessageCircle,"Inbox","/messages"],[ShoppingBag,cart?`Bag ${cart}`:"Bag","/cart"]] .map(([Icon,label,href])=><Link key={String(label)} href={String(href)} className={(p===href||(href!=="/"&&p.startsWith(String(href))))?"active":""}><span className={label==="Sell"?"sell":""}>{typeof Icon!=="string"&&<Icon/>}</span><small>{label}</small></Link>)}</nav>}

export default function Marketplace(){
  const path=usePathname(); const router=useRouter();
  const [lang,setLang]=usePersistent<Lang>("velora-language","sq");
  const [signedIn,setSignedIn]=usePersistent<boolean>("velora-signed-in",false);
  ...
  const signOut=()=>{clearPrivate();setSignedIn(false)};
  ...
  else if(path==="/sell")content=<SellPage signedIn={signedIn} publish={p=>setListings([p,...listings])}/>;

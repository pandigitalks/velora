...
import {
  ...
  Fingerprint, HelpCircle, ImagePlus, LayoutDashboard, LockKeyhole, LogOut, Mail, MapPin, Menu, MessageCircle,
...
type Product = {
  ...
  material?: string; gender?: string; shipping?: boolean; negotiable?: boolean;
};
...
const products: Product[] = [
  ...
  {id:18,name:"Jet Set leather tote",brand:"MICHAEL KORS",price:190,oldPrice:245,image:"/assets/bag-one.webp",position:"35% center",seller:"Second Muse",size:"Një madhësi",city:"Berlin",verified:false,label:"Ulje çmimi",category:"Bags",color:"E zezë",condition:"Shkëlqyeshëm",authLevel:"none",material:"Lëkurë",gender:"Femra",shipping:true,negotiable:true},
...
function AuthBadge({p,compact=false}:{p:Product;compact?:boolean}){
  ...
  const data={
    ...
    ai:["Analiza AI u përfundua",p.authRisk==="medium"?"Rrezik mesatar falsifikimi":p.authRisk==="high"?"Rrezik i lartë falsifikimi":"Rrezik i ulët falsifikimi"],
    expert:["Rishikuar nga eksperti","Kërko raportin e kontrollit"],
    physical:["Kontrolluar fizikisht","Raporti fizik është i disponueshëm"]
  }[level];
...
function requestAccount(next:string){
  window.dispatchEvent(new CustomEvent("velora:auth-required",{detail:{next}}));
}
...
function AppHeader({cartCount,noteCount,notes,setNotes,lang,setLang,signedIn,onSignIn,onSignOut}:{cartCount:number;noteCount:number;notes:Note[];setNotes:(v:Note[])=>void;lang:Lang;setLang:(l:Lang)=>void;signedIn:boolean;onSignIn:()=>void;onSignOut:()=>void}){
  ...
  useEffect(()=>{
    const openAuth=(event:Event)=>{
      ...
      setAfterLogin(next?.startsWith("/")&&!next.startsWith("//")?next:null);
      setProfile(false);setMenu(false);setNotifications(false);setAuth(true);
    };
  ...
  return <>
    ...
    <AnimatePresence>{menu&&<motion.div className="v2-drawer-bg v2-menu-bg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setMenu(false)}><motion.aside className="v2-mobile-menu" initial={{x:"-100%"}} animate={{x:0}} exit={{x:"-100%"}} onClick={e=>e.stopPropagation()}><div><Brand/><button className="v2-icon" aria-label="Mbyll menynë" onClick={()=>setMenu(false)}><X/></button></div>{[["Eksploro","/explore",false],["Brendet","/brands",false],["Histori","/stories",false],["Autentikimi","/authentication",false],["Shitës profesionalë","/professional-sellers",false],["Të ruajturat","/saved",true],["Shporta","/cart",true],["Mesazhet","/messages",true],["Porositë","/orders",true],["Profili","/profile",true],["Cilësimet","/settings",true],["FAQ","/faq",false],["Kontakti","/contact",false]].map(([label,href,protectedItem])=>protectedItem&&!signedIn?<button className="v2-mobile-menu-link" onClick={()=>requestAccount(String(href))} key={String(href)}>{label}<ChevronRight/></button>:<Link onClick={()=>setMenu(false)} key={String(href)} href={String(href)}>{label}<ChevronRight/></Link>)}<div className="v2-mobile-language"><span>{lang==="sq"?"Gjuha":"Language"}</span><div className="v2-lang-choice"><button className={lang==="sq"?"active":""} onClick={()=>setLang("sq")}>Shqip</button><button className={lang==="en"?"active":""} onClick={()=>setLang("en")}>English</button></div>{signedIn?<button className="v2-mobile-logout" onClick={()=>{setMenu(false);onSignOut();router.push("/")}}><LogOut/>{lang==="sq"?"Dil nga llogaria":"Sign out"}</button>:<button className="v2-mobile-logout" onClick={()=>{setMenu(false);setAfterLogin(null);setAuth(true)}}>{lang==="sq"?"Kyçu":"Sign in"}</button>}</div></motion.aside></motion.div>}</AnimatePresence>
...
function ExplorePage({saved,toggle,add,listings=[],signedIn}:{saved:number[];toggle:(id:number)=>void;add:(id:number)=>void;listings?:ListingDraft[];signedIn:boolean}){
  const router=useRouter();
  const params=useSearchParams();
  const [filters,setFilters]=useState(false);
  const [aiSearch,setAiSearch]=useState(false);
  const [aiQuery,setAiQuery]=useState("");
  const [savedSearches,setSavedSearches]=usePersistent<string[]>("velora-saved-searches",[]);
  const q=(params.get("q")||"").toLowerCase();
  const category=params.get("category")||"All";
  const sort=params.get("sort")||"curated";
  const active={
    brand:params.get("brand")||"All",condition:params.get("condition")||"All",city:params.get("city")||"All",
    gender:params.get("gender")||"All",size:params.get("size")||"All",color:params.get("color")||"All",
    material:params.get("material")||"All",min:Number(params.get("min"))||0,max:Number(params.get("max"))||10000,
    auth:params.get("auth")||"All",shipping:params.get("shipping")==="1",negotiable:params.get("negotiable")==="1",
  };
  const [draft,setDraft]=useState(active);
  const userProducts=useMemo<Product[]>(()=>listings.map((l,i)=>({id:l.id||90000+i,name:l.title,brand:(l.brand||"VELORA USER").toUpperCase(),price:l.price,image:l.image||"/assets/bag-one.webp",position:"center",seller:"Arnis Archive",size:l.size||"Një madhësi",city:"Pejë",verified:false,label:"E re",category:l.category||"Bags",color:"—",condition:l.condition||"Mirë",authLevel:"none",gender:l.gender,shipping:true,negotiable:true})),[listings]);
  const catalog=useMemo(()=>[...userProducts,...products],[userProducts]);
  const value=(p:Product,key:"gender"|"material")=>p[key]||(key==="gender"?(p.category==="Women"?"Femra":"Unisex"):(p.category==="Bags"?"Lëkurë":p.category==="Jewelry"?"Metal":"Tekstil"));
  const results=useMemo(()=>catalog.filter(p=>(category==="All"||p.category===category)&&(active.brand==="All"||p.brand===active.brand)&&(active.condition==="All"||p.condition===active.condition)&&(active.city==="All"||p.city===active.city)&&(active.gender==="All"||value(p,"gender")===active.gender)&&(active.size==="All"||p.size===active.size)&&(active.color==="All"||p.color===active.color)&&(active.material==="All"||value(p,"material")===active.material)&&(!q||`${p.name} ${p.brand} ${p.seller} ${p.category} ${p.color}`.toLowerCase().includes(q))&&p.price>=active.min&&p.price<=active.max&&(active.auth==="All"||p.authLevel===active.auth)&&(!active.shipping||p.shipping!==false)&&(!active.negotiable||p.negotiable!==false)).sort((a,b)=>sort==="low"?a.price-b.price:sort==="high"?b.price-a.price:sort==="drop"?(b.oldPrice?1:0)-(a.oldPrice?1:0):sort==="popular"?Number(b.verified)-Number(a.verified):b.id-a.id),[catalog,category,q,sort,active.brand,active.condition,active.city,active.gender,active.size,active.color,active.material,active.min,active.max,active.auth,active.shipping,active.negotiable]);
  const setParam=(key:string,value:string)=>{const next=new URLSearchParams(params.toString());if(!value||value==="All")next.delete(key);else next.set(key,value);router.push(`/explore${next.toString()?`?${next}`:""}`)};
  const applyFilters=()=>{const next=new URLSearchParams(params.toString());Object.entries(draft).forEach(([key,raw])=>{const string=String(raw);if(raw===false||string==="All"||(key==="min"&&raw===0)||(key==="max"&&raw===10000))next.delete(key);else next.set(key,typeof raw==="boolean"?"1":string)});router.push(`/explore${next.toString()?`?${next}`:""}`);setFilters(false)};
  const clearFilters=()=>{router.push("/explore");setDraft({brand:"All",condition:"All",city:"All",gender:"All",size:"All",color:"All",material:"All",min:0,max:10000,auth:"All",shipping:false,negotiable:false})};
  const filterCount=Object.entries(active).filter(([key,v])=>v!=="All"&&v!==false&&!(key==="min"&&v===0)&&!(key==="max"&&v===10000)).length;
  const selectOptions=(key:"brand"|"condition"|"city"|"size"|"color",field:keyof Product)=>[...new Set(catalog.map(p=>String(p[field]||"")).filter(Boolean))].sort();
  const saveSearch=()=>{if(!signedIn){requestAccount(`/explore${params.toString()?`?${params}`:""}`);return}const key=params.toString()||"Të gjitha produktet";if(!savedSearches.includes(key))setSavedSearches([...savedSearches,key])};
  const runAiSearch=(e:FormEvent)=>{e.preventDefault();if(!aiQuery.trim())return;router.push(`/explore?q=${encodeURIComponent(aiQuery.trim())}`);setAiSearch(false)};
  return <main className="v2-page">
    <div className="v2-explore-head"><div><span>EKSPLORO</span><h1>{q?`Rezultatet për “${params.get("q")}”`:"Pjesë që ia vlen t’i zbulosh"}</h1><p>{results.length===1?"1 produkt i përzgjedhur":`${results.length} produkte të përzgjedhura`}</p></div><div className="v2-explore-tools"><button onClick={()=>setAiSearch(true)}><Sparkles/><span><b>Kërko me VELORA AI</b><small>Provo “çantë elegante nën 800€”</small></span><ArrowRight/></button><button className="v2-save-search" onClick={saveSearch}><Bell/>{savedSearches.includes(params.toString()||"Të gjitha produktet")?"Kërkimi u ruajt":"Ruaj kërkimin"}</button></div></div>
    <div className="v2-filterbar"><div>{categories.map(c=><button key={c} className={category===c?"active":""} onClick={()=>setParam("category",c)}>{c}</button>)}</div><div><button onClick={()=>{setDraft(active);setFilters(true)}}><Filter/>Filtrat{filterCount>0&&<b>{filterCount}</b>}</button><select value={sort} onChange={e=>setParam("sort",e.target.value)} aria-label="Rendit produktet"><option value="curated">Të përzgjedhura</option><option value="new">Më të rejat</option><option value="popular">Më të pëlqyerat</option><option value="low">Çmimi: nga i ulëti</option><option value="high">Çmimi: nga i larti</option><option value="drop">Uljet më të mëdha</option></select></div></div>
    {filterCount>0&&<div className="v2-active-filters">{Object.entries(active).map(([key,v])=>v!=="All"&&v!==false&&!(key==="min"&&v===0)&&!(key==="max"&&v===10000)?<button key={key} onClick={()=>setParam(key,"All")}>{key==="min"?`Nga ${v}€`:key==="max"?`Deri ${v}€`:key==="shipping"?"Me transport":key==="negotiable"?"I negociueshëm":String(v)}<X/></button>:null)}<button className="clear" onClick={clearFilters}>Pastro të gjitha</button></div>}
    {results.length?<div className={`v2-grid v2-results ${results.length===1?"single":""}`}>{results.map(p=><ProductCard key={p.id} p={p} saved={saved} toggle={toggle} add={add}/>)}</div>:<div className="v2-empty"><Search/><h2>Nuk u gjet asnjë produkt</h2><p>Pastro filtrat ose provo një kërkim më të gjerë.</p><button className="v2-pill dark" onClick={clearFilters}>Pastro filtrat</button></div>}
    <AnimatePresence>{filters&&<motion.div className="v2-drawer-bg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setFilters(false)}><motion.aside className="v2-filter-drawer" role="dialog" aria-modal="true" aria-label="Filtrat" initial={{x:"100%"}} animate={{x:0}} exit={{x:"100%"}} onClick={e=>e.stopPropagation()}><div className="v2-drawer-head"><div><small>PERSONALIZO REZULTATET</small><h2>Filtrat</h2></div><button className="v2-icon" onClick={()=>setFilters(false)} aria-label="Mbyll filtrat"><X/></button></div><div className="v2-filter-scroll"><div className="v2-filter-grid"><label>Brendi<select value={draft.brand} onChange={e=>setDraft({...draft,brand:e.target.value})}><option value="All">Të gjitha</option>{selectOptions("brand","brand").map(x=><option key={x}>{x}</option>)}</select></label><label>Gjendja<select value={draft.condition} onChange={e=>setDraft({...draft,condition:e.target.value})}><option value="All">Të gjitha</option>{selectOptions("condition","condition").map(x=><option key={x}>{x}</option>)}</select></label><label>Gjinia<select value={draft.gender} onChange={e=>setDraft({...draft,gender:e.target.value})}><option value="All">Të gjitha</option>{["Femra","Meshkuj","Unisex","Fëmijë"].map(x=><option key={x}>{x}</option>)}</select></label><label>Madhësia<select value={draft.size} onChange={e=>setDraft({...draft,size:e.target.value})}><option value="All">Të gjitha</option>{selectOptions("size","size").map(x=><option key={x}>{x}</option>)}</select></label><label>Ngjyra<select value={draft.color} onChange={e=>setDraft({...draft,color:e.target.value})}><option value="All">Të gjitha</option>{selectOptions("color","color").map(x=><option key={x}>{x}</option>)}</select></label><label>Materiali<select value={draft.material} onChange={e=>setDraft({...draft,material:e.target.value})}><option value="All">Të gjitha</option>{["Lëkurë","Tekstil","Metal","Mëndafsh","Lesh","Acetat"].map(x=><option key={x}>{x}</option>)}</select></label><label>Lokacioni<select value={draft.city} onChange={e=>setDraft({...draft,city:e.target.value})}><option value="All">Të gjitha</option>{selectOptions("city","city").map(x=><option key={x}>{x}</option>)}</select></label><label>Niveli i kontrollit<select value={draft.auth} onChange={e=>setDraft({...draft,auth:e.target.value})}><option value="All">Të gjitha</option><option value="ai">Analizë AI</option><option value="expert">Rishikim eksperti</option><option value="physical">Kontroll fizik</option><option value="none">Pa kontroll</option></select></label></div><fieldset className="v2-price-filter"><legend>Çmimi</legend><label><span>Minimumi</span><div>€<input inputMode="numeric" value={draft.min} onChange={e=>setDraft({...draft,min:Math.max(0,Number(e.target.value.replace(/\D/g,"")))})}/></div></label><label><span>Maksimumi</span><div>€<input inputMode="numeric" value={draft.max} onChange={e=>setDraft({...draft,max:Math.max(draft.min,Number(e.target.value.replace(/\D/g,"")))})}/></div></label></fieldset><label className="v2-switch-row"><span><b>Transport i disponueshëm</b><small>Vetëm produkte që mund të dërgohen</small></span><input type="checkbox" checked={draft.shipping} onChange={e=>setDraft({...draft,shipping:e.target.checked})}/></label><label className="v2-switch-row"><span><b>Çmim i negociueshëm</b><small>Shitësi pranon oferta</small></span><input type="checkbox" checked={draft.negotiable} onChange={e=>setDraft({...draft,negotiable:e.target.checked})}/></label></div><footer><button className="v2-link-btn" onClick={clearFilters}>Pastro</button><button className="v2-pill dark" onClick={applyFilters}>Shfaq rezultatet</button></footer></motion.aside></motion.div>}</AnimatePresence>
    <AnimatePresence>{aiSearch&&<motion.div className="v2-modal-bg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setAiSearch(false)}><motion.form className="v2-ai-search-modal" onSubmit={runAiSearch} initial={{opacity:0,y:20,scale:.98}} animate={{opacity:1,y:0,scale:1}} exit={{opacity:0,y:20,scale:.98}} onClick={e=>e.stopPropagation()}><button type="button" className="v2-icon close" onClick={()=>setAiSearch(false)} aria-label="Mbyll"><X/></button><Sparkles/><span>VELORA AI SEARCH</span><h2>Përshkruaje atë që kërkon.</h2><p>Kërko me stil, buxhet, brend, ngjyrë ose rast përdorimi.</p><div><Search/><input autoFocus value={aiQuery} onChange={e=>setAiQuery(e.target.value)} placeholder="p.sh. çantë e zezë elegante nën 800€"/></div><section>{["Atlete të reja EU 43","Orë luksoze nën 2,000€","Çantë vintage në Prishtinë"].map(x=><button type="button" key={x} onClick={()=>setAiQuery(x)}>{x}</button>)}</section><button className="v2-pill dark wide">Kërko tani<ArrowRight/></button></motion.form></motion.div>}</AnimatePresence>
  </main>
}

function ListingPage({id,saved,toggle,add,buy,catalog,signedIn}:{id:number;saved:number[];toggle:(id:number)=>void;add:(id:number)=>void;buy:(id:number)=>void;catalog:Product[];signedIn:boolean}){
  const router=useRouter(); const p=catalog.find(x=>x.id===id)||products[0]; const [offer,setOffer]=useState(false); const [sent,setSent]=useState(false); const [question,setQuestion]=useState(""); const [report,setReport]=useState(false);
  return <main className="v2-page v2-listing"><div className="v2-breadcrumb"><Link href="/explore">Discover</Link><ChevronRight/><Link href={`/explore?category=${p.category}`}>{p.category}</Link><ChevronRight/><span>{p.name}</span></div><div className="v2-listing-layout">
    <section className="v2-gallery single-photo"><div className="main"><ProductImage p={p}/><span><Camera/>1 fotografi · publikuar nga shitësi</span></div></section>
    <aside className="v2-buybox"><div className="v2-buy-brand">{p.brand}<button className={saved.includes(p.id)?"on":""} onClick={()=>toggle(p.id)} aria-label="Ruaje produktin"><Heart fill={saved.includes(p.id)?"currentColor":"none"}/></button></div><h1>{p.name}</h1><p>{p.size} · {p.condition} · {p.color}</p><h2>€{p.price.toLocaleString()}<small>Mbrojtja e blerësit përfshihet</small></h2><button className="v2-auth-report-trigger" onClick={()=>setReport(true)}><AuthBadge p={p}/><span>Shiko statusin<ChevronRight/></span></button><button className="v2-pill dark wide" onClick={()=>buy(p.id)}>Bli tani · €{p.price.toLocaleString()}</button><button className="v2-pill outline wide" onClick={()=>signedIn?setOffer(true):requestAccount(`/listing/${p.id}`)}>Bëj ofertë</button><button className="v2-pill soft wide" onClick={()=>add(p.id)}><ShoppingBag/>Shto në shportë</button><p className="v2-demand"><Clock3/>{18+p.id*3} persona e kanë ruajtur</p><div className="v2-buy-essentials"><details open><summary>Detajet kryesore<Plus/></summary><div>{[["Materiali",p.material||"Nuk është specifikuar"],["Madhësia",p.size],["Ngjyra",p.color],["Referenca",`VL-${55000+p.id}`]].map(x=><p key={x[0]}><span>{x[0]}</span><b>{x[1]}</b></p>)}</div></details><details><summary>Çfarë përfshihet<Plus/></summary><div><p><span>Përmbajtja e paketës</span><b>Kontrollo përshkrimin</b></p></div></details><details><summary>Dërgesa dhe kthimi<Plus/></summary><div><p><span>Dërgesë e siguruar</span><b>4–7 ditë</b></p><p><span>Kthimi</span><b>Brenda 48 orëve</b></p><p><span>Niset nga</span><b>{p.city}</b></p></div></details></div><Link href={`/seller/${encodeURIComponent(p.seller.toLowerCase().replace(/[^a-z0-9]+/g,"-"))}`} className="v2-seller"><span className="v2-avatar">{p.seller.slice(0,2).toUpperCase()}</span><span><b>{p.seller}{p.verified&&<BadgeCheck/>}</b><small><Star fill="currentColor"/>4.9 · 128 shitje · dërgon për 1.2 ditë</small></span><ChevronRight/></Link></aside>
  </div><section className="v2-product-deep"><div><span>DETAJET E PRODUKTIT</span><h2>Gjithçka para se të vendosësh.</h2><p>Informacioni është dhënë nga shitësi. Statusi i kontrollit tregon qartë nivelin e provave të disponueshme.</p><div className="v2-detail-grid">{[["Brendi",p.brand],["Modeli",p.name],["Gjendja",p.condition],["Materiali",p.material||"Nuk është specifikuar"],["Ngjyra",p.color],["Madhësia",p.size],["Referenca",`VL-${55000+p.id}`],["Publikuar","Së fundmi"]].map(x=><article key={x[0]}><small>{x[0]}</small><b>{x[1]}</b></article>)}</div><h3>Përshkrimi</h3><p>Fotografia paraqet produktin e listuar. Për hollësi shtesë, kërko nga shitësi fotografi të etiketës, brendësisë dhe shenjave të përdorimit.</p></div><aside><article><Package/><span><b>Dërgesë e siguruar</b><small>4–7 ditë · gjurmim i plotë</small></span></article><article><CreditCard/><span><b>Detyrimet para pagesës</b><small>Tarifat dhe importi llogariten para konfirmimit</small></span></article><article><LockKeyhole/><span><b>Pagesë e mbrojtur</b><small>Shitësi paguhet pas dorëzimit</small></span></article><article><RefreshCcw/><span><b>Kthim brenda 48 orëve</b><small>Nëse produkti ndryshon nga shpallja</small></span></article><div className="v2-seller-stats"><h3>{p.seller}</h3><p><span><b>128</b><small>Shitje</small></span><span><b>96%</b><small>Dërguar</small></span><span><b>1.2 ditë</b><small>Koha mesatare</small></span><span><b>0.8%</b><small>Anulime</small></span></p><Link href={`/seller/${encodeURIComponent(p.seller.toLowerCase().replace(/[^a-z0-9]+/g,"-"))}`}>Shiko profilin<ArrowRight/></Link></div></aside></section><section className="v2-price-history"><div><span>VLERËSIM ORIENTUES</span><h2>Krahasim i çmimit</h2><p>Vlerësimi është orientues dhe nuk përfaqëson histori shitjesh të verifikuara.</p></div><div className="v2-price-bars">{[70,82,76,91,86,64].map((h,i)=><i key={i} style={{height:h}}><span>{["Shk","Mar","Pri","Maj","Qer","Sot"][i]}</span></i>)}</div><aside><small>REFERENCA ORIENTUESE</small><b>€{Math.round(p.price*1.08).toLocaleString()}</b><small>ÇMIMI AKTUAL</small><strong>€{p.price.toLocaleString()}</strong></aside></section><section className="v2-comments"><div className="v2-section-head"><div><span>PYETJE PËR PRODUKTIN</span><h2>Kontakto shitësin</h2></div></div><p className="v2-question-note">Pyet për gjendjen, përmasat, aksesorët ose kërko fotografi shtesë.</p><button className="v2-pill dark" onClick={()=>signedIn?router.push("/messages"):requestAccount(`/listing/${p.id}`)}>Dërgo mesazh<Send/></button></section><Rail title="Mund të të pëlqejnë edhe" items={catalog.filter(x=>x.id!==p.id).slice(0,4)} saved={saved} toggle={toggle} add={add}/>
  ...
  <AnimatePresence>{report&&<motion.div className="v2-modal-bg" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setReport(false)}><motion.div role="dialog" aria-modal="true" className="v2-auth-report" initial={{y:24,opacity:0}} animate={{y:0,opacity:1}} exit={{y:24,opacity:0}} onClick={e=>e.stopPropagation()}><button className="v2-icon close" onClick={()=>setReport(false)} aria-label="Mbyll raportin"><X/></button><span>STATUSI I KONTROLLIT</span><h2>Vlerësim transparent, jo garanci absolute.</h2><AuthBadge p={p}/><p>{p.authLevel==="none"?"Ky produkt nuk ka ende analizë AI ose kontroll manual. Kërko fotografi shtesë dhe prova para blerjes.":"Analiza vlerëson rrezikun nga provat e disponueshme dhe nuk duhet interpretuar si deklaratë 100% autenticiteti."}</p><div className="v2-auth-evidence"><article><b>Klasifikimi</b><span>{p.authLevel==="none"?"Dëshmi të pamjaftueshme":p.authRisk==="high"?"Rrezik i lartë falsifikimi":p.authRisk==="medium"?"Rrezik mesatar falsifikimi":"Rrezik i ulët falsifikimi"}</span></article><article><b>Rishikimi manual</b><span>{p.authLevel==="none"||p.authRisk==="high"?"Kërkohet":"Sipas nevojës"}</span></article></div><Link className="v2-auth-learn" href="/authentication">Si funksionon sistemi VELORA?<ArrowRight/></Link></motion.div></motion.div>}</AnimatePresence>
  </main>
}
...
function SellerProfilePage({slug,saved,toggle,add}:{slug:string;saved:number[];toggle:(id:number)=>void;add:(id:number)=>void}){
  const sellerProducts=products.filter(p=>p.seller.toLowerCase().replace(/[^a-z0-9]+/g,"-")===slug);
  const seller=sellerProducts[0]?.seller||slug.split("-").map(x=>x.charAt(0).toUpperCase()+x.slice(1)).join(" ");
  const initials=seller.split(/\s+/).map(x=>x[0]).join("").slice(0,2).toUpperCase();
  return <main className="v2-page"><section className="v2-profile-head public"><div className="v2-profile-avatar">{initials}</div><div><span>PROFIL PUBLIK I SHITËSIT</span><h1>{seller}</h1><p>Shitës në VELORA · përgjigjet zakonisht brenda një ore</p><div><b>{sellerProducts.length}<small>Shpallje</small></b><b>4.9<small>Vlerësimi</small></b><b>128<small>Shitje</small></b></div></div><aside><button className="v2-pill dark" onClick={()=>requestAccount("/messages")}><MessageCircle/>Kontakto</button></aside></section><div className="v2-profile-trust"><span><ShieldCheck/>Pagesat mbrohen nga VELORA</span></div><div className="v2-tabs"><button className="active">Garderoba</button><button>Vlerësimet</button></div>{sellerProducts.length?<div className="v2-grid v2-results">{sellerProducts.map(p=><ProductCard key={p.id} p={p} saved={saved} toggle={toggle} add={add}/>)}</div>:<Empty icon={<Package/>} title="Nuk ka shpallje aktive" text="Ky shitës nuk ka produkte aktive për momentin." href="/explore" action="Eksploro produktet"/>}</main>
}
...
function MobileNav({cart:_,signedIn}:{cart:number;signedIn:boolean}){
  ...
  const items=[
    ...
    {Icon:MessageCircle,label:"Mesazhe",href:"/messages",protected:true},
    {Icon:User,label:"Profili",href:"/profile",protected:true},
  ];
  return <nav className="v2-mobile-nav" aria-label="Navigimi kryesor">{items.map(({Icon,label,href,protected:needsAccount})=>{
    const content=<><span className={label==="Shit"?"sell":""}><Icon/></span><small>{label}</small></>;
    const active=p===href||(href!=="/"&&p.startsWith(href));
    return needsAccount&&!signedIn?<button type="button" key={label} className={active?"active":""} onClick={()=>requestAccount(href)} aria-label={`Kyçu për ${label.toLowerCase()}`}>{content}</button>:<Link key={label} href={href} className={active?"active":""}>{content}</Link>;
  })}</nav>
}
...
export default function Marketplace(){
  ...
  useEffect(()=>{
    const supabase=createClient();
    void supabase.auth.getSession().then(({data})=>{setSignedIn(Boolean(data.session?.user));if(!data.session?.user)setAccount(null);setAuthReady(true)});
    void supabase.auth.getUser().then(({data})=>{setSignedIn(Boolean(data.user));if(!data.user)setAccount(null)}).catch(()=>{setSignedIn(false);setAccount(null)});
    const {data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{setSignedIn(Boolean(session?.user));if(!session?.user)setAccount(null);setAuthReady(true)});
  ...
  const signOut=async()=>{await createClient().auth.signOut();clearPrivate();setAccount(null);setSignedIn(false)};
  ...
  if(path==="/explore"||path==="/search")content=<ExplorePage saved={privateSaved} toggle={toggle} add={add} listings={signedIn?listings:[]} signedIn={signedIn}/>;
  else if(path.startsWith("/listing"))content=<ListingPage id={id} saved={privateSaved} toggle={toggle} add={add} buy={buy} catalog={signedIn?listingCatalog:products} signedIn={signedIn}/>;
  ...
  else if(path.startsWith("/seller/"))content=<SellerProfilePage slug={path.split("/").pop()||""} saved={privateSaved} toggle={toggle} add={add}/>;
